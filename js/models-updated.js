...
const Models = {
  ...
  User: {
    ...
    /**
     * Actualiza un usuario existente con campos de seguridad
     * @param {string} id - ID del usuario
     * @returns {Object} Usuario actualizado
     */
    upgradeUserSecurity: function(id) {
      ...
      // Verificar si el usuario ya tiene campos de seguridad
      if (user.securityFields) {
        return user;
      }
      
      // Extender el usuario con campos de seguridad
      if (typeof UserSecurity !== 'undefined') {
        const updatedUser = UserSecurity.extendUserModel(user);
        
        // Si el usuario tiene contraseña en texto plano, hashearla
        if (updatedUser.password) {
          try {
            const hashResult = UserSecurity.hashPassword(updatedUser.password);
            if (hashResult && hashResult.then) {
              // Es una promesa, manejarla adecuadamente
              hashResult.then(result => {
                updatedUser.securityFields.passwordHash = result.hash;
                updatedUser.securityFields.passwordSalt = result.salt;
                updatedUser.securityFields.lastPasswordChange = new Date().toISOString();
              ...
              updatedUser.securityFields.passwordHash = hashResult.hash;
              updatedUser.securityFields.passwordSalt = hashResult.salt;
              updatedUser.securityFields.lastPasswordChange = new Date().toISOString();
            ...
            // Actualizar el usuario de todas formas
            this.update(id, updatedUser);
          }
        } else {
          // Actualizar el usuario sin hashear contraseña
          this.update(id, updatedUser);
        }
    ...
    /**
     * Actualiza todos los usuarios con campos de seguridad
     * @returns {number} Número de usuarios actualizados
     */
    upgradeAllUsersSecurity: function() {
      const users = this.getAll();
      let updatedCount = 0;
      
      users.forEach(user => {
        try {
          this.upgradeUserSecurity(user.id);
          updatedCount++;
        } catch (error) {
          console.error(`Error al actualizar seguridad del usuario ${user.id}:`, error);
        }
      });
  ...
  Menu: {
    ...
    getActiveMenu: function() {
      ...
      // Filtrar menús activos (no archivados)
      ...
      activeMenus.sort((a, b) => new Date(b.weekStart) - new Date(a.weekStart));
    ...
    create: function(menuData) {
      ...
      const newMenu = {
        id: Utils.generateId(),
        title: menuData.title || `Menú Semana ${new Date(menuData.weekStart).toLocaleDateString()}`,
    ...
    _createEmptyWeekDays: function() {
      ...
      startDate.setDate(startDate.getDate() - startDate.getDay()); // Ajustar al domingo
  ...
  Confirmation: {
    ...
    getByCoordinatorAndMenu: function(coordinatorId, menuId) {
      ...
      return confirmations.find(conf => 
        conf.coordinatorId === coordinatorId && conf.menuId === menuId
      ) || null;
    },
    ...
    create: function(confirmationData) {
      ...
      const existingConfirmation = this.getByCoordinatorAndMenu(
      ...
      const newConfirmation = {
        ...
        weekStart: confirmationData.weekStart,
